 ...
 * @author dov
 */
public class AffineSpacePlaneBipartate_1 {
    ...
    public class ASNode {
        ...
        public ASNode(AffineSpace as) {
            affineSpace = as;
            planes = new ArrayList<>(as.b.dim());
            for (int i = 0; i < as.b.dim(); i++)
                planes.add(new PlaneNode(Plane(as.linearSpace().normals[i], as.b.get(i))));
        }
...
//            else System.out.println("in");
        ...
        public List<ASNode> oneDown() {
            ...
            return affineSpace.oneDown()
                    .map(as -> affineSpaceNodes.getOrDefault(as, new ASNode(as)))
    ...
    public AffineSpacePlaneBipartate_1(int dim) {
     ...
     * @return
     */
    public List<? extends ASNode> affineSpaces(ArrayList<ASNode> lowerLevel) {
        if (lowerLevel.isEmpty())
            return planeNodes
                    .values()
                    .stream()
                    .map(pn -> pn.affineSpaces.iterator().next())
                    .collect(Collectors.toList());

        return lowerLevel
                .stream()
                .parallel()
                .flatMap(as -> planeNodes.values().stream()
                .filter(planeNode -> !as.planes.contains(planeNode))
                .map(pn -> {
                    List<PlaneNode> planes = new ArrayList<>(as.planes.size() + 1);
                    planes.addAll(as.planes);
                    planes.add(pn);
                    return new ASNode(pn.plane.intersection(as.affineSpace), planes);
                })
                )
                .collect(Collectors.toList());
    }
