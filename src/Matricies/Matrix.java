...
public class Matrix {
    ...
    /**
     * creates a new matrix with a column swapped out.
     *
     * @param i the column to be replaced.
     * @param p the replacement column
     * @return the new matrix
     */
    public Matrix swapColumn(int i, double[] p) {
    ...
    public Matrix systemSol(Matrix b) {
        Matrix x = new Matrix(b.rows, b.cols);

        IntStream.range(0, cols).parallel().forEach(i -> x.setCol(i, solve(b.col(i))));
     ...
     *
     * @param err the end condition
     * @return e^X
     */
    public Matrix exp(double err) {
        Matrix a_n = Matrix.identityMatrix(rows),
                A = Matrix.identityMatrix(rows),
                last = new Matrix(rows, rows);
        long kFactorial = 1;
        for (long k = 1; Math.abs(A.minus(last).det()) > err; k++) {
            kFactorial *= k;
            a_n = a_n.mult(this);
            last = A;
            A = A.plus(a_n.mult(1.0 / kFactorial));
        }
        return A;
    }
    ...
    /**
     * Constructor
     * @param array a single array representation of a 2d concept.  
     * The first col elements go in the first row, 
     * the second col elements go in the second row, and so on.  
     * Make sure that rows*cols = array.length;
    ...
    public Point crossProduct() {
        if (cols != 1 + rows)
            throw new RuntimeException("this matrix is the wrong size for cross "
                    + "product. You must have cols == rows + 1 but cols = " + cols
                    + " and rows = " + rows);
    ...
    public Matrix rowConcat(Matrix rows) {
        ...
        double[] rowConcatArray = Arrays.copyOf(array, array.length + rows.array.length);
    ...
    public Matrix colConcat(Matrix cols) {
        Matrix concat = new Matrix(Math.max(this.rows, cols.rows), this.cols + cols.cols);
        return concat.setCols(i -> i < this.cols ? col(i) : cols.col(i - this.cols));
    }
    ...
    public Matrix independentColumns() {
        Matrix ind = new Matrix(rows, cols - reducedRowEchelon().numFreeVariables());
...
//     * @return
//     */
//    public Point solve(double[] b) {
//        return new Point(rows).setAll(i -> swapColumn(i, b).det() / det());
//    }
//
//    public Point solve(Point b) {
//        return new Point(solve(b.array()));
//    }
//
//    /**
//     * A stream of rows
...
//        Matrix x = new Matrix(b.rows, b.cols);
//
//        IntStream.range(0, cols).parallel().forEach(
//                i -> x.setCol(i, solve(b.col(i))));
//
//        return x;
//    }
//
...
//    public Matrix mult(Matrix A) {
//        if (cols != A.rows) return null;
//        return new Matrix(rows, A.cols)
//                .setAll((i, j) -> row(i).dot(A.col(j)));
//    }
//
//    /**
//     * multiplies the matrix by a constant.
...
//        if (cols == 1) return array[0];
//
//        return IntStream.range(0, cols).parallel()
//                .mapToDouble(i -> Math.pow(-1, i) * get(0, i) * minor(0, i).det())
//                .sum();
//    }
//
//    /**
...
//        double det = det();
//        if (det == 0)
//            throw new ArithmeticException("\nThis matrix:\n" + toString() + " has no inverse. det = " + det);
//
//        Matrix m = new Matrix(rows, cols);
//
//        if (rows > 1)
//            m.setAll((i, j) -> Math.pow(-1, i + j) * minor(i, j).det());
//        else return m.set(0, 0, 1.0 / det);
//
...
//     */
//    public Pair1T<Matrix> QRDecomposition() {
//        Matrix E = triangulizationMatrix();
//        return new Pair1T<>(E.inverse(), E.mult(this));
//    }
//
//    /**
//     * creates a new Matrix from a set of rows
...
//     */
//    public Matrix setCols(ZToRn f) {
//        IntStream.range(0, cols).parallel().forEach(i -> setCol(i, f.of(i)));
//        return this;
//    }
//
//    /**
...
//    }
//
//    public long rank(double epsilon) {
//        return myReducedRowEchelon(epsilon).rank();
//    }
//
//    /**
//     * Creates a new matrix whose rows are appended to this one
...
//     * this matrix
//     */
//    public Matrix independentColumns(double epsilon) {
//        Matrix ind = new Matrix(rows, cols - myReducedRowEchelon(epsilon).numFreeVariables());
//        for (int to = 0, from = 0; from < cols; from++)
//            if (!myReducedRowEchelon(epsilon).isFreeVariabls(from))
//                ind.setCol(to++, col(from));
//        return ind;
//    }
//
...
//     */
//    public Matrix independentRows(double epsilon) {
//        return T().independentColumns(epsilon).T();
//    }
//
//    /**
//     * A new matrix without some of the rows
...
//    private ReducedRowEchelon rre = null;
//
//    public ReducedRowEchelon myReducedRowEchelon(double epsilon) {
//        if (rre == null) return rre = new ReducedRowEchelon(this, epsilon);
//        else return rre;
//    }
//
//    public List<Point> colList() {
